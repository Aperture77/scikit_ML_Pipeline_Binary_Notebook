...
from sklearn.impute import (
    IterativeImputer,
)  # a.k.a MICE - Recommended advanced method / Industry standard
...
def cv_partitioner(
    ...
    datasetList = list(
        list(x) for x in zip(*(td[x].values.tolist() for x in td.columns))
    )
    ...
    # Stratified Partitioning Method-----------------------
        ...
        if categoricalOutcome:  # Discrete outcome
            ...
            print(
                "Error: Stratified partitioning only designed for discrete endpoints. "
            )
        ...
        if categoricalOutcome:
            ...
            for (
                matchSet
            ) in byMatchRows:  # Go through each unique set of matched instances
    ...
    # Determinist Partitioning Method-----------------------
        ...
        if categoricalOutcome:
            ...
            byMatchRows = [
                [] for i in range(len(matchList))
            ]  # create list of empty lists (one train one for test)
            for row in datasetList:
                # find index in matchList corresponding to the matchset of the current row.
                mIndex = matchList.index(row[matchIndex])
                row.pop(matchIndex)  # remove match column from partition output
            ...
            print(
                "Error: Determinist partitioning only designed for discrete endpoints. "
            )

    # Matched-Determinist Partitioning Method-----------------------
    elif partition_method == "MD":
        if categoricalOutcome:
            # Get train/test (match) variable column index
            outcomeIndex = td.columns.get_loc(outcomeLabel)
            matchIndex = td.columns.get_loc(matchName)
            partIndex = td.columns.get_loc(partName)

            print("Matched-Determinist Partitioning")
            # Create data sublists for train and test data
            matchList = ["train", "test"]
            partList = np.sort(td[partName].unique())  # Get Groupe Names and sort them
            part_grouped = td.groupby(partName)  # Group the dataframe by CV number
            cv_df_list = []
            for i in partList:
                cv_df_list.append(part_grouped.get_group(i))  # Split DF by CV Group

            train_dfs = []
            test_dfs = []
            for df_i in cv_df_list:  # Now for each CV group split by train/test matched
                matched_grouped = df_i.groupby(matchName)
                train_dfs.append(
                    matched_grouped.get_group("train").drop(
                        [matchName, partName], axis=1
                    )
                )
                test_dfs.append(
                    matched_grouped.get_group("test").drop(
                        [matchName, partName], axis=1
                    )
                )

            header.pop(matchIndex)  # remove match column from partition output
            header.pop(partIndex)  # remove part column from partition output
    ...
    for each in train_dfs:
        ...
        if instLabel == None or instLabel == "None":
            scale_train_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_train, columns=[outcomeLabel]),
                        pd.DataFrame(x_train_scaled, columns=header),
                    ],
                    axis=1,
                    sort=False,
                )
            ...
            scale_train_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_train, columns=[outcomeLabel]),
                        pd.DataFrame(inst_train, columns=[instLabel]),
                        pd.DataFrame(x_train_scaled, columns=header),
                    ],
                    axis=1,
                    sort=False,
        ...
        if instLabel == None or instLabel == "None":
            scale_test_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_test, columns=[outcomeLabel]),
                        pd.DataFrame(x_test_scaled, columns=header),
                    ],
                    axis=1,
                    sort=False,
                )
            ...
            scale_test_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_test, columns=[outcomeLabel]),
                        pd.DataFrame(inst_test, columns=[instLabel]),
                        pd.DataFrame(x_test_scaled, columns=header),
                    ],
                    axis=1,
                    sort=False,
        ...
        if instLabel == None or instLabel == "None":
            imp_train_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_train, columns=[outcomeLabel]),
                        pd.DataFrame(x_new_train, columns=header),
                    ],
                    axis=1,
                    sort=False,
                )
        ...
        if instLabel == None or instLabel == "None":
            imp_test_dfs.append(
                pd.concat(
                    [
                        pd.DataFrame(y_test, columns=[outcomeLabel]),
                        pd.DataFrame(x_new_test, columns=header),
                    ],
                    axis=1,
                    sort=False,
                )
