...
def eval_Algorithm_FI(
    ...
    FI_ave = [0] * len(
        ordered_feature_names
    )  # Holds only the selected feature FI results for each partition
    ...
    for i in tqdm_notebook(range(cv_partitions), desc="eval_Algorithm_FI"):
        ...
        alg_result_table.append(
            [fpr, tpr, roc_auc, recall, prec, prec_rec_auc, ave_prec]
        )
        ...
        for each in ordered_feature_names:
            if (
                each in global_ordered_features[i]
    ...
    plt.legend(loc="best")
    plt.savefig(
        (wd_path + output_folder + "/" + algorithm + "_ROC_" + data_name),
        bbox_inches="tight",
    )
    plt.show()
    ...
    for i in range(cv_partitions):
        ...
        plt.plot(
            ...
            lw=1,
            alpha=0.3,
            label="PRC fold %d (AUC = %0.2f) (APS = %0.2f)"
            % (i, alg_result_table[i][5], alg_result_table[i][6]),
        )

    # Add chance line to PRC plot
    noskill = len(yTestList[0][yTestList[0] == 1]) / len(
        yTestList[0]
    )  # Fraction of cases
    plt.plot(
        [0, 1],
    ...
    plt.legend(loc="best")
    plt.savefig(
        (wd_path + output_folder + "/" + algorithm + "_PRC_" + data_name),
        bbox_inches="tight",
    )
    plt.show()
    ...
    print(
        "Avg. PRC Precision Score = "
        + str(np.mean(aveprecs))
        + " (std. dev. = "
        + str(np.std(aveprecs))
        + ")"
    )
...
def objective_GB(
    ...
    params = {
        ...
        "n_iter_no_change": trial.suggest_int(
            "n_iter_no_change",
            param_grid["n_iter_no_change"][0],
            param_grid["n_iter_no_change"][1],
        ),
        "validation_fraction": trial.suggest_discrete_uniform(
            "validation_fraction",
            param_grid["validation_fraction"][0],
            param_grid["validation_fraction"][1],
            param_grid["validation_fraction"][2],
        ),
    }
    return hyper_eval(est, x_train, y_train, randSeed, hype_cv, params, scoring_metric)
...
def objective_ExSTraCS(
    trial, est, x_train, y_train, randSeed, hype_cv, param_grid, scoring_metric
):  # Should I factor in scores here
    params = {
        "learning_iterations": trial.suggest_categorical(
            "learning_iterations", param_grid["learning_iterations"]
        ),
